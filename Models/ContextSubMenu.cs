...
public class ContextSubMenu(List<ContextMenuItem>? items = null,
     SolidColorBrush? icon = null, string? name = null, Action<ContextMenuItem, Control?>? itemAction = null) : ContextMenuItem(icon, name, itemAction)
{
    public readonly List<ContextMenuItem> items = items ?? [];
}

public class ContextSubMenu<T>(List<ContextMenuItem<T>>? items = null, SolidColorBrush? icon = null, string? name = null,
